...
/**
 * Vendored from React Bits (`@react-bits/Hyperspeed-TS-TW`), unmodified except:
 *   - the "use client" directive above,
 *   - the wrapper div uses a class instead of a hardcoded `id="lights"`,
 *   - `init()`/`dispose()` are guarded against a disposed instance (see below).
 *
 * The guards fix a hard crash: `init()` runs after `loadAssets()` resolves, so a
 * mount/unmount inside that window — React StrictMode's double-invoke in dev is
 * the reliable way to hit it — disposes the renderer first. `addPass()` then
 * reads `getContextAttributes()` on a force-lost context, gets null, and throws
 * "Cannot read properties of null (reading 'alpha')".
 *
 * Colours are supplied by the caller — see `components/chrome/HyperspeedBackdrop.tsx`,
 * which retunes the traffic to the ember/signal palette. `effectOptions` must be
 * a stable module-level object: the effect below rebuilds the entire scene
...
class App {
  ...
  init() {
    // Local patch: `loadAssets()` resolved after this instance was torn down, so
    // the renderer's context is gone. Building passes against it would throw.
  ...
  dispose() {
    // Local patch: idempotent. The effect below can dispose the same instance
    // twice, and `forceContextLoss()` on an already-lost context is not safe.
    if (this.disposed) return;
    this.disposed = true;
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  return <div className="hyperspeed-canvas" ref={hyperspeed}></div>;
};
